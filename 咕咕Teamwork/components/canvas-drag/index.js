...
// 增加了绘制线条的类
const edgeGraph = function({width = 2,color = 'black'},canvas,fromGraph,toGraph){
  this.ctx = canvas;
...
Component({
    ...
    methods: {
        ...
        addNew(fromGraph){
          ...
          fromGraph.childs.push(newgraph);
          var newedge = new edgeGraph({width:2,color:'black'},this.ctx,fromGraph,newgraph);
        ...
        _insertNode(fromgraph,graph){
          ...
          if(fromgraph.childs.length === undefined){
            fromgraph.childs = [];
            reutrn;
          }
          for(var i = 0;i<fromgraph.childs.length;i++){
            var title = fromgraph.childs[i].title;
            // TODO：这里简单写了一下树结点初始的位置变化，比较粗糙，可以根据textSize动态改变间距和文字大小之类的
            var newgraph = new dragGraph({ x: graph.x + pos_x_offset, y: graph.y + pos_y_offset, text:title, type: "text" }, this.ctx, this.factor);
            // 把新的graph push进父graph的childs里
            graph.childs.push(newgraph);
            // push进组件画布的drawArr
            this.drawArr.push(newgraph);
            var newedge = new edgeGraph({ width: 2, color: 'black' }, this.ctx, graph, newgraph);
            this.edgeArr.push(newedge);
            pos_x_offset += 100;
            // 递归 
            this._insertNode(fromgraph.childs[i], newgraph);
          }
        },
